...
def _value_failure_summary(
    ...
    economy: str = "",
    include_anchor_review_split: bool = False,
) -> pd.DataFrame:
    ...
    if failures_source.empty or not required.issubset(failures_source.columns):
        return pd.DataFrame()
    failures = _filter_dashboard_economy(failures_source, economy)
    failures = failures[failures["status"].astype(str).eq("failed")].copy()
    if failures.empty:
        return pd.DataFrame()
    context_columns = [
        "source_system", "validation_axis", "economy", "scenario", "year",
    ...
    if include_anchor_review_split and _has_explicit_anchor_review_fields(failures):
        failures["_confirmed_issue_failed"] = _confirmed_source_issue_mask(failures)
        failures["_unconfirmed_failed"] = ~failures["_confirmed_issue_failed"]
        failures["_source_non_additivity_observed"] = (
            failures.get(
                "source_non_additivity_observed",
                pd.Series(False, index=failures.index),
            )
            .astype(str)
            .str.lower()
            .eq("true")
...
def _filter_dashboard_economy(frame: pd.DataFrame, economy: str) -> pd.DataFrame:
    """Limit an artifact to the dashboard economy when the artifact has that axis."""
    if frame.empty or not economy or "economy" not in frame.columns:
        return frame.copy()
    economy_key = str(economy).replace("_", "").strip()
    economy_values = (
        frame["economy"].astype(str).str.replace("_", "", regex=False).str.strip()
    )
...
def _confirmed_source_issue_mask(frame: pd.DataFrame) -> pd.Series:
    ...
    confirmed = (
        frame["known_data_quality_exception"].astype(str).str.lower().eq("true")
    )
    confirmed &= (
        frame["exception_review_status"].astype(str).str.lower().eq("confirmed")
    )
    return confirmed


def _has_explicit_anchor_review_fields(frame: pd.DataFrame) -> bool:
    """Return whether the artifact can distinguish confirmed review metadata."""
    return {
...
def _anchor_value_summary(anchor: pd.DataFrame, economy: str = "") -> pd.DataFrame:
    """Sum failed source-to-mapped-frontier comparisons."""
    return _value_failure_summary(
        anchor,
        "frontier_sum",
        economy=economy,
        include_anchor_review_split=True,
    )


def _reviewed_anchor_exceptions(anchor: pd.DataFrame, economy: str) -> pd.DataFrame:
    """Show explicit source-issue confirmations independently of check status."""
    if anchor.empty or "known_data_quality_exception" not in anchor.columns:
        return pd.DataFrame()
    scoped = _filter_dashboard_economy(anchor, economy)
    flagged = scoped[_confirmed_source_issue_mask(scoped)].copy()
    columns = [
        "source_system", "validation_axis", "parent_code", "other_axis_value",
        "economy", "scenario", "year", "status", "parent_value", "frontier_sum",
        "difference", "abs_error", "reason", "source_non_additivity_observed",
        "exception_id", "exception_review_status", "exception_issue_class",
        "exception_resolution", "data_quality_exception_notes",
    ]
...
def _paired_tree_html(
    ...
    economy: str,
    economy_examples: pd.DataFrame | None = None,
    ...
    for row in summary.head(12).itertuples(index=False):
        ...
        case_evidence_html = _case_evidence_html(
            economy_examples if economy_examples is not None else pd.DataFrame(),
            review_candidates if review_candidates is not None else pd.DataFrame(),
            source_system=source_system,
            validation_axis=str(row.validation_axis),
            parent_code=str(row.parent_code),
            other_axis_value=str(row.other_axis_value),
        )
        cards.append(
            ...
            f'</ul>{mapped_structure_note}{frontier_basis_note}{detail_shortfall_note}</section></div>'
            f'{case_evidence_html}</article>'
        )
    if not cards:
...
def _exception_classification_description(classification: str) -> str:
    """Explain the review meaning of an upstream exception class briefly."""
    normalized = str(classification).strip().lower()
    if normalized.endswith("source_non_additivity"):
        return (
            "The raw source parent and its declared children do not reconcile. "
            "The mismatch is present in the source hierarchy, not created by the Common ESTO mapping."
        )
    descriptions = {
        "intentional_detail_exclusion": (
            "The shared comparison boundary deliberately omits source detail that is outside its cross-dataset scope, "
            "so the raw total is not expected to match the mapped frontier."
        ),
        "provisional_apec_anchor_review": (
            "This is a temporary APEC-wide review record. The case is hidden from the default queue, "
            "but still needs a final root-cause classification."
        ),
        "unclassified_exception": (
            "The exception set matched this case but did not supply an issue class. "
            "Use the evidence and upstream review fields to classify it."
        ),
    }
    return descriptions.get(
        normalized,
        "This upstream exception class has no dashboard-specific description yet. "
        "Use the evidence and upstream review fields below for its authoritative context.",
    )
...
def _paired_tree_exception_browser_html(
    ...
    groups_html = "".join(
        ...
        f'<p class="exception-class-description">{escape(_exception_classification_description(classification))}</p>'
        f'{content}</section>'
...
def _case_evidence_html(
    ...
    *,
    ...
    """Put matched economy and review evidence beside the failed check it explains."""
    match_values = {
        "source_system": source_system,
    ...
    def matched_rows(frame: pd.DataFrame) -> pd.DataFrame:
        if frame.empty or not set(match_values).issubset(frame.columns):
            return pd.DataFrame()
        matched = frame.copy()
        ...
        if "comparison_scope" in matched.columns and context_columns:
            matched["_scope_priority"] = matched["comparison_scope"].map(SCOPE_PRIORITY).fillna(99)
            matched = (
                matched.sort_values("_scope_priority", kind="mergesort")
                .drop_duplicates(context_columns, keep="first")
                .drop(columns="_scope_priority")
            )
        return matched

    evidence = matched_rows(economy_examples)
    if not evidence.empty:
        status_rank = evidence.get("status", pd.Series("", index=evidence.index)).map(
            {"failed": 0, "passed": 1, "skipped": 2}
        ).fillna(3)
        evidence["_status_rank"] = status_rank
        evidence["_abs_error_sort"] = pd.to_numeric(
            evidence.get("abs_error", 0), errors="coerce"
        ).fillna(0.0)
        evidence = evidence.sort_values(
            ["_status_rank", "_abs_error_sort"], ascending=[True, False], kind="mergesort"
        ).drop(columns=["_status_rank", "_abs_error_sort"])
        failed_count = int(evidence.get("status", pd.Series("", index=evidence.index)).astype(str).eq("failed").sum())
        supporting_count = int(len(evidence) - failed_count)
        evidence_summary = (
            f"Why did this fail? Economy evidence ({failed_count} failed, "
            f"{supporting_count} supporting rows)"
        )
        evidence_table = _table_html(evidence, [
            "economy", "scenario", "year", "status", "parent_value", "frontier_sum",
            "difference", "abs_error", "reason", "attribution_status",
            "source_non_additivity_observed", "known_data_quality_exception",
            "exception_id", "exception_review_status", "exception_issue_class",
            "data_quality_exception_notes",
        ])
    else:
        evidence_summary = "Why did this fail? Economy evidence unavailable"
        evidence_table = (
            '<p class="empty-state">No matched economy rows were published for this check.</p>'
        )

    candidates = matched_rows(review_candidates)
    candidate_html = ""
    if not candidates.empty:
        candidate_html = (
            '<h4>Source review candidates</h4>'
            '<p class="subtle">These are review prompts, not confirmed exceptions.</p>'
            + _table_html(candidates, [
                "economy", "scenario", "year", "parent_value", "direct_children_sum",
                "leaf_descendants_sum", "candidate_classification", "notes",
            ])
        )
    return (
        f'<details class="case-evidence"><summary>{escape(evidence_summary)}</summary>'
        '<div><p class="subtle">Matched rows are kept with this failure so you can start with '
        'the comparison above, then inspect which economies and years account for it.</p>'
        f'{evidence_table}{candidate_html}</div></details>'
    )
...
def write_mapping_diagnostics_page(
    ...
    *,
    ...
    mapping_download_source_path = results_root / "common_esto" / "source_to_common_esto_map.csv"
    mapping_download_path = layout["supporting"] / "source_to_common_esto_map.csv"
    if mapping_download_source_path.is_file():
        shutil.copy2(mapping_download_source_path, mapping_download_path)
    ...
    stage_failed = (
        scoped_stage["status"].astype(str).eq("failed")
        if "status" in scoped_stage.columns
        else pd.Series(False, index=scoped_stage.index)
    )
    anchor_failed = (
        scoped_anchor["status"].astype(str).eq("failed")
        if "status" in scoped_anchor.columns
        else pd.Series(False, index=scoped_anchor.index)
    )
    has_anchor_review_fields = _has_explicit_anchor_review_fields(scoped_anchor)
    anchor_confirmed = _confirmed_source_issue_mask(scoped_anchor)
    anchor_review_metrics = (
        [
            {
                "metric": "Confirmed source issues among failed anchor rows",
                "rows": int((anchor_failed & anchor_confirmed).sum()),
            },
            {
                "metric": "Unconfirmed failed anchor rows",
                "rows": int((anchor_failed & ~anchor_confirmed).sum()),
            },
        ]
        if has_anchor_review_fields
        else [
            {
                "metric": "Failed anchor rows without explicit review fields",
                "rows": int(anchor_failed.sum()),
            },
        ]
    )
    summary = pd.DataFrame([
        {"metric": "Failed final-output hierarchy detail rows", "rows": int(stage_failed.sum())},
        {"metric": "Failed anchor detail rows", "rows": int(anchor_failed.sum())},
        *anchor_review_metrics,
        {"metric": "Leaf-reconciliation exception candidates", "rows": int(len(scoped_leaf_reconciliation_candidates))},
    ...
    dashboard_guide = build_guide_fragments(
        "diagnostics",
        "mapping_diagnostics",
        "APEC-wide mapping diagnostics",
    )
    html = f"""<!DOCTYPE html>
...
.diagnostic-header-actions {{ display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; }} .mapping-download {{ margin-left:auto; color:#1b5e9a; font-weight:700; }}
...
.exception-browser-controls {{ position:sticky; top:0; z-index:2; background:white; padding:4px 0 12px; }} .exception-browser-controls label {{ display:grid; gap:4px; max-width:420px; color:#5f6b7a; font-size:12px; }} .exception-browser-controls select {{ padding:7px; }} .exception-class-group {{ border-top:1px solid #d9e1ea; padding-top:16px; margin-top:12px; }} .exception-class-group code {{ color:#334155; }} .exception-class-description {{ max-width:920px; margin:0 0 16px; padding:9px 11px; border-left:3px solid #94a3b8; background:#f8fafc; color:#334155; }}
...
{dashboard_guide["css"]}</style></head><body><div class="shell"><header><div class="diagnostic-header-actions"><a href="#" onclick="history.back();return false;">← Back to economy dashboard</a>{dashboard_guide["launch_button_html"]}<a class="mapping-download" href="../supporting_files/source_to_common_esto_map.csv" download>Download complete mappings (CSV)</a></div><h1>APEC-wide mapping diagnostics</h1><p class="subtle">Structural checks start from the summed APEC balance. Economy evidence is calculated only for failed APEC anchors. Updated: {escape(dashboard_updated_label)}<br>{escape(contract_note)}</p></header>
...
<section class="panel"><p class="source-warning"><strong>LEAP ESTO Extended cases are currently suppressed.</strong> {suppressed_leap_extended_case_count:,} LEAP anchor contexts contain child mappings that belong to an ESTO Extended scope. They are excluded until the detailed LEAP sectors are fully imported into the main LEAP areas; the page does not treat their absent detail as a mapping failure.</p></section>
<section class="panel"><h2>Imported LEAP category recognition</h2><p class="subtle">LEAP labels are retained after known aliases are normalised. A category is marked for review here when a non-zero LEAP flow/product pair has no direct maintained LEAP-to-ESTO mapping; this does not mean that its source label is invalid. <strong>{len(unmapped):,}</strong> non-zero pair(s) need review. Open <strong>Direct mapping coverage review → Non-zero unmapped LEAP branches</strong> for the pairs and any indirect ESTO lead.</p></section>
...
(() => {
  ...
  const roots = ordinaryNodes
    ...
    .map(node => node.code)
    .sort();
  const includeExtended = document.querySelector('#include-extended-rows');
  ...
  const allCommonRowsForSource = source => {
    const result = new Set();
    const sources = source === 'ESTO'
      ? ['ESTO', 'ESTO_RAW']
      : source === 'ESTO_EXTENDED' ? ['ESTO_EXTENDED', 'ESTO_EXTENDED_RAW'] : [source];
    ROLLUP_VALUES
      .filter(row => sources.includes(row.source_system))
      .forEach(row => result.add(row.common_flow_label));
    return result;
  };
  ...
  const originFor = code => {
    const inEsto = allCommonRowsForSource('ESTO').has(code);
    const inExtended = allCommonRowsForSource('ESTO_EXTENDED').has(code);
    ...
    return 'Outside ESTO structures';
  };
  const validationSources = () => [rs.value];
  const nodeStatus = node => {
    ...
    const hasValue = valuesForSource(rs.value).has(node.code);
    return hasValue
  ...
  const codeAvailableForBasis = code => {
    if (includeExtended.checked) return true;
    const estoRows = allCommonRowsForSource('ESTO');
  ...
  const refreshSelectors = () => {
    ...
    const availableSources = !includeExtended.checked
    ...
    const sourceRows = ROLLUP_VALUES.filter(row => row.source_system === rs.value);
    ...
    basisState.textContent = includeExtended.checked
      ? 'Including ESTO Extended-only rows'
      : 'Showing original ESTO structure';
    renderGraph();
  };

  fill(sector, ['ALL', ...roots], 'ALL');
  sector.options[0].text = 'All major sectors (collapsed)';
  includeExtended.addEventListener('change', refreshSelectors);
...
"""
    ...
    html = html.replace(
        ...
        + dashboard_guide["dialog_html"]
        + f'<script>{dashboard_guide["script"]}</script></body></html>',
    )
