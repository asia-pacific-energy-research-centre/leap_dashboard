...
| Medium | Product stacks can contain more categories than any categorical palette can uniquely and accessibly distinguish. | Use a dark, muted 20-colour palette that avoids lime/neon shades on white. Keep trace membership unchanged; colours repeat only after the available distinct options. | Implemented |
