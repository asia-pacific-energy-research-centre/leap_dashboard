...
| DASHQ-025 | P1 | 2026-08-03 to 2026-08-10 | `not_started` | DASHQ-007 | Resolve the publication blocker on empty transfers charts | `scripts/check_common_esto_dashboard_publish_ready.py` **currently fails**, blocking publication. 27 failures, all one class: 3 transfers charts (`chart__area__08__08_transfers`, and the `transfers__flow` / `transfers__product` section charts) render with no Plotly traces across 9 economies — 02BD, 05PRC, 08JPN, 11MEX, 12NZ, 13PNG, 14PE, 18CT, 21VN. Confirmed pre-existing on 2026-07-28 and unrelated to `f514b1f`; economies rendered both before and after that change fail identically. Decide whether these economies genuinely have no transfers data (in which case the chart should be suppressed rather than emitted empty) or whether the upstream rows are missing. Complete when the readiness check passes or each remaining failure has a recorded, intentional reason. |
