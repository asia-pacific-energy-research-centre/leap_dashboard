...
| DASHQ-059 | P1 | 2026-08-20 | `not_started` | `leap_initialisation` estimation methods; DASHQ-058 | Agentic estimation-method chart review using dashboard figures | Create and maintain an active instruction pack that lets an agent discover documented estimation methods, run their read-only calculations as tests against selected LEAP exports, and render only material, safe differences through the production dashboard chart builders. The agent must preserve input/method/export/mapping provenance, treat transformation own-use and placeholder boundaries explicitly, and withhold unsafe comparisons rather than allocate or invent expected values. The initial visual reference is `C:\Users\Work\.codex\visualizations\2026\08\20\01a01cc6-7099-7342-a3b4-4aa05b55e2bd\transformation_net_fuel_balance.html`; its manually authored Plotly output is evidence of the desired review question, not an implementation to copy. |
