...
| DASHQ-037 | P2 | 2026-08-17 | `complete_on_master` | — | Add a colleague-facing dashboard colour workbook round trip | Added a macro-free Excel editor with one visible combined category code/name column, visibly filled colour cells, typed-hex or paint-bucket editing (including Excel theme colours), strict import validation, a persistent custom-config layer, focused tests, and an operator guide. Verification: 122 dashboard/workbook tests passed; all six user-facing sheets passed visual QA; a 20USA fixture render wrote 941 charts across both comparison bases; publication readiness passed and page-noise analysis reported zero flags. |
