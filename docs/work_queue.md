...
| DASHQ-055 | P2 | 2026-08-16 | `planned` | Initialisation queue [44], mapping MAPQ-047 | Migrate machine-only dashboard intermediates to typed columnar storage | Inventory renderer-side inputs, caches and staging tables and benchmark Parquet+Zstandard against their current formats using real selected-column/filter access. Migrate only server/Python-side machine artifacts with producer-consumer and render-equivalence tests. Preserve browser-facing JSON/HTML bundles, human-readable manifests/readiness reports and the current Common ESTO contract until all consumers have a versioned replacement. The portable/web runtime must prove dependency closure before receiving Parquet. |
