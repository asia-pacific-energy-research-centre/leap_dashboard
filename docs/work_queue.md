...
| DASHQ-060 | P1 | 2026-08-20 | `mapping_complete_dashboard_verification_pending` | `leap_mappings` Common ESTO rollups; DASHQ-056, DASHQ-059 | Include LNG demand-side own use in LEAP inclusive gas-processing boundaries | Mapping-side fix completed in `leap_mappings`: both LEAP inclusive rollups now include `Demand\Other loss and own use\Liquefaction and regasification plants` with multiplier `-1`, focused/broader mapping tests pass, and a renewed bounded AUS Common ESTO contract verifies the 2023 Target inclusive row as natural gas `-4528.757899`, LNG `+4220.411097`, and electricity `-28.073591`, while the standalone `10.01.03` row remains visible. Stage 3 mapped-value conservation remained 100%. Remaining work is dashboard rerender/normal-chart and shadow-review verification before removing the temporary caveat. |
