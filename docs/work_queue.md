...
| DASHQ-042 | P1 | 2026-08-31 | `in_progress` | DASHQ-041, upstream AUS fixture owner | Remove redundant combined bunker presentation and audit detailed non-road/Power inputs | Detailed Supply now keeps only the separate marine and aviation Overview cards when both are available; combined 04-05 product/flow cards remain only as the fallback for placeholder data. Source audit found that non-road incorrectly subtracts 51.012955 PJ of already-separated international activity in 2023. Both power workbooks contain only retained Electricity/CHP interim branches, not genuine plant detail. Upstream correction and fixture regeneration remain in progress; no dashboard-side data adjustment is permitted. |
