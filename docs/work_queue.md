...
| DASHQ-037 | P2 | 2026-08-17 | `complete_on_master` | — | Add a colleague-facing dashboard colour workbook round trip | Added a macro-free Excel editor with only three visible sheets (`START HERE`, `Products`, and `Flows`), one combined category code/name column, and one visibly filled editable colour cell. Mapping-owned rolled Common ESTO categories use equal-weight OKLab averages of their ESTO component colours by default and can be overridden in the same sheet. Import supports typed hex values, paint-bucket/theme colours, strict validation, and a persistent custom-config layer. Verification: all 266 tests passed; all three user-facing sheets passed visual QA with no formula errors; a fresh 20USA fixture render wrote 941 charts across both comparison bases and confirmed the averaged rollup colour in generated bundles; publication readiness passed and page-noise analysis reported zero flags. |
