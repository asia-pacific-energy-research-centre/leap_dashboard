...
| DASHQ-067 | P1 | 2026-08-31 | `implemented_pending_production_render` | Common ESTO hierarchy contract | Pair useful first- and second-level aggregate charts by product and by flow | Aggregate cards are now emitted only when at least two immediate child flows are non-zero, except for explicitly configured boundary summaries such as non-energy, imports, exports, refining and energy-balance totals. The Power overview owns paired summaries for generation, power-related losses and own use, and own use; duplicate lower-page ownership is suppressed. Renderer regressions cover the two-child threshold, configured exceptions, page scoping, and Power ownership; 202 focused renderer tests pass. Real AUS placeholder and detailed-road production renders remain the release gate. |
