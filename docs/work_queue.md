...
| DASHQ-060 | P1 | 2026-08-20 | `not_started` | `leap_mappings` Common ESTO rollups; DASHQ-056, DASHQ-059 | Include LNG demand-side own use in LEAP inclusive gas-processing boundaries | Correct the Common ESTO mapping/rollup contributor set for `09.06.02 Liquefaction/regasification plants (including own use)` and parent `09.06 Gas processing plants (including own use)`: LEAP must include the mapped `Demand\Other loss and own use\Liquefaction and regasification plants` result (`10.01.03`) alongside the `09.06.02` transformation result, by product and with its negative signed fuel areas visible. ESTO, LEAP, and 9th must resolve through the same inclusive boundary; the dashboard must not compensate with a renderer-only total-line adjustment. Implement in `leap_mappings`, add source-specific contributor and base-year/projection hand-off regressions, rerun the mapping pipeline, publish/validate the renewed Common ESTO contract, then rerender dashboard outputs and verify the AUS 2023 Target inclusive LEAP total against the transformed LEAP result plus its demand-proxy result. Regenerate the shadow review from that renewed contract; remove its temporary LEAP-boundary caveat only after the normal chart stack, total line, and mapped contributors agree. |
