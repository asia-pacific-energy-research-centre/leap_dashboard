...
<div class="box"><label>Source tree <select id="choose"></select></label><label>Year <select id="year"></select></label><label>Scenario <select id="scenario"></select></label><label>Magnitude <select id="magnitude"><option value="1">PJ</option><option value="1000">Thousands of PJ</option><option value="1000000">Millions of PJ</option></select></label><label>Find node <input id="find" placeholder="code or label"></label><span id="count" class="muted"></span></div>
...
<script>const T={tree_json},R={route_json},N={numeric_json},L=document.querySelector('#left'),Z=document.querySelector('#right'),C=document.querySelector('#common'),S=document.querySelector('#choose'),Y=document.querySelector('#year'),V=document.querySelector('#scenario'),G=document.querySelector('#magnitude'),Q=document.querySelector('#find'),sys={{leap:'LEAP',ninth:'NINTH',esto:'ESTO'}};let active=[],lastNode=null;
...
function selectNode(n){{lastNode=n;let [d,a]=S.value.split('|'),found=R.filter(r=>r.source_system===sys[d]&&((a==='flow'||a==='sector')?r.original_source_flow===n.code:r.original_source_product===n.code)),ids=[...new Set(found.map(r=>r.common_row_id).filter(Boolean))],flows=new Set(found.map(r=>r.component_esto_flow)),commonFlows=new Set(found.map(r=>r.common_flow_label)),unique=new Map(),divisor=Number(G.value),unit=G.options[G.selectedIndex].text;found.forEach(r=>{{let val=N.filter(x=>x.source_system===r.source_system&&x.structural_scope===r.comparison_scope&&x.common_row_id===r.common_row_id&&String(x.year)===Y.value&&(V.value==='all'||x.scenario===V.value)).reduce((s,x)=>s+Number(x.value),0);if(r.common_row_id&&!unique.has(r.common_row_id))unique.set(r.common_row_id,val);r.selected_total=val}});let total=[...unique.values()].reduce((a,b)=>a+b,0),format=v=>(v/divisor).toLocaleString(undefined,{{maximumFractionDigits:4}});draw(active,L,new Set([n.code]),true);draw(T.filter(x=>x.dataset==='esto'&&x.axis==='flow'),Z,flows,false);draw(T.filter(x=>x.dataset==='common_esto'&&x.axis==='flow'),C,commonFlows,false);document.querySelector('#detail-title').textContent=name(n)+' — mapping routes';document.querySelector('#note').textContent=`${{found.length}} routes; ${{ids.length}} unique Common ESTO row IDs; selected total ${{format(total)}} ${{unit}}. Totals sum all economies for the selected year/scenario.`;let rows=found.slice(0,500).map(r=>`<tr><td>${{r.original_source_flow}}</td><td>${{r.original_source_product}}</td><td>${{r.component_esto_flow}} / ${{r.component_esto_product}}</td><td>${{r.common_row_id}}</td><td>${{format(r.selected_total)}}</td><td>${{r.comparison_scope}}</td></tr>`).join('');document.querySelector('#detail').innerHTML=rows?`<table><thead><tr><th>Source flow</th><th>Source product</th><th>ESTO component</th><th>Common row ID</th><th>Selected total (${{unit}})</th><th>Scope</th></tr></thead><tbody>${{rows}}</tbody></table>${{found.length>500?'<p class="muted">First 500 routes shown.</p>':''}}`:'<p class="muted">No registered structural route for this node.</p>'}}
